...
public class PrescriptionsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetByAppointmentId(int appointmentId)
    {
        ...
        if (prescription == null)
            return NotFound(
                new ErrorResponseDTO { Message = "Prescription not found", StatusCode = 404 }
            );
    ...
    [HttpPost("suggest")]
    [Authorize(Roles = "Doctor")]
    public async Task<IActionResult> Suggest([FromBody] CreatePrescriptionDTO dto)
    {
        if (!ModelState.IsValid)
            return BadRequest(ModelState);
        ...
        try
        {
            var result = await _service.CreateSuggestionAsync(dto);
            // return CreatedAtAction(
            //     nameof(GetByAppointmentId),
            //     new { appointmentId = result.AppointmentId },
            //     result
