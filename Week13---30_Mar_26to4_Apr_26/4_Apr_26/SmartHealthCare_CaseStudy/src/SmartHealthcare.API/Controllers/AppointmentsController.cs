...
public class AppointmentsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        if (appt == null)
            return NotFound(
                new ErrorResponseDTO { Message = "Appointment not found", StatusCode = 404 }
            );
        return Ok(appt);
    }
    ...
    )
    {
        ...
        if (doctor == null)
            return BadRequest(
                new ErrorResponseDTO { Message = "Doctor profile not found", StatusCode = 400 }
            );
    ...
    public async Task<IActionResult> Create([FromBody] CreateAppointmentDTO dto)
    {
        ...
        _logger.LogInformation(
            "Appointment booking initiated by PatientId: {PatientId}",
            patient.Id
        );
