...
public class AdminController : ControllerBase
{
    ...
    // This constructor gets access to all the below services using dependency injection. We register these services in progra.cs and whenever someone needs it they can just access them from here.
    ...
    public async Task<IActionResult> UpdateRole(int id, [FromBody] UpdateUserRoleDTO dto)
    {
        ...
        if (user == null)
            return NotFound(new ErrorResponseDTO { Message = "User not found", StatusCode = 404 });
        user.Role = dto.Role;
    ...
    public async Task<IActionResult> GetReports()
    {
        ...
        if (!_cache.TryGetValue(cacheKey, out object? report))
        {
            report = new
            {
                ...
                CompletedAppointments = await _context.Appointments.CountAsync(a =>
                    a.Status == "Completed"
                ),
