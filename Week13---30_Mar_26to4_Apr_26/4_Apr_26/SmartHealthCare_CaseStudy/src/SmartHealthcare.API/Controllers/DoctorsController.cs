...
public class DoctorsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        if (doctor == null)
            return NotFound(
                new ErrorResponseDTO { Message = "Doctor not found", StatusCode = 404 }
            );
        return Ok(doctor);
    }

    [HttpGet("search")]
    public async Task<IActionResult> SearchBySpecialization(
    ...
    )
    {
        if (string.IsNullOrEmpty(specialization))
            return BadRequest(
                new ErrorResponseDTO { Message = "Specialization is required", StatusCode = 400 }
            );

        var result = await _service.SearchBySpecializationAsync(
            specialization,
            pageNumber,
            pageSize
        );
    ...
    public async Task<IActionResult> GetMyProfile()
    {
        ...
        if (doctor == null)
            return NotFound(
                new ErrorResponseDTO { Message = "Profile not found", StatusCode = 404 }
            );
        return Ok(doctor);
    }
